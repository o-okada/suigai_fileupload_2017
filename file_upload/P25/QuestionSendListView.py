#####################################################################################
# QuestionSendListViewビューモジュール
# ファイル名：P25.QuestionSendListView.py（Ｐ２５）
# ユースケース：都道府県の担当者は、問合せまたは回答の送信一覧を閲覧する。
# ユースケース：本省の担当者は、問合せまたは回答の送信一覧を閲覧する。
# ユースケース：運用業者の担当者は、問合せまたは回答の送信一覧を閲覧する。
...
#####################################################################################
# 関数名：P25.QuestionSendListView（Ｐ２５Ａ）
# 関数概要：都道府県用問合せまたは回答の送信一覧ページをブラウザに戻す。（都道府県）
# 関数概要：本省用問合せまたは回答の送信一覧ページをブラウザに戻す。（本省）
# 関数概要：運用業者用問合せまたは回答の送信一覧ページをブラウザに戻す。（運用業者）
#
# 引数[1]：request
...
# 戻り値[1]：response
#
# FORM：QuestionSendListForm：都道府県用問合せまたは回答の送信一覧ページ（都道府県）
# FORM：QuestionSendListForm：本省用問合せまたは回答の送信一覧ページ（本省）
# FORM：QuestionSendListForm：運用業者用問合せまたは回答の送信一覧ページ（運用業者）
# MODEL：AccountModel：アカウントモデル（Ｒ）
# MODEL：QuestionModel：問合せまたは回答モデル（Ｒ）
#####################################################################################
@login_required(None, login_url='/file_upload/P01/Login/')
def QuestionSendListView(request, accountType, accountId, operationYear, filterAccount, filterRead):
    try:
        print_log('[INFO] ##########################################', 'INFO')
        print_log('[INFO] P25.QuestionSendListView関数が開始しました。', 'INFO')
        ##########################################
        # 引数チェック処理（Ｐ２５Ａ１０）
        # （１）ブラウザからのリクエストと引数をチェックする。
        ##########################################
        # （１）ブラウザからのリクエストと引数をチェックする。
        print_log('[INFO] P25.QuestionSendListView関数 P25A10', 'INFO')
        print_log('[INFO] P25.QuestionSendListView.method = {}'.format(request.method), 'INFO')
        print_log('[INFO] P25.QuestionSendListView.accountType = {}'.format(accountType), 'INFO')
        print_log('[INFO] P25.QuestionSendListView.accountId = {}'.format(accountId), 'INFO')
        print_log('[INFO] P25.QuestionSendListView.operationYear = {}'.format(operationYear), 'INFO')
        print_log('[INFO] P25.QuestionSendListView.filterAccount = {}'.format(filterAccount), 'INFO')
        print_log('[INFO] P25.QuestionSendListView.filterRead = {}'.format(filterRead), 'INFO')
        ##########################################
        # 引数チェック処理（Ｐ２５Ａ２０）
        ...
        if accountId == str(request.user.username):
            ...
            print_log('[WARN] P25.QuestionSendListView関数 P25A20-1', 'WARN')
            return render(request, 'error.html')
        # （２）リクエストメソッドをチェックする。　例　GET、POST
        if request.method == 'GET':
            pass
        else:
            print_log('[WARN] P25.QuestionSendListView関数 A25A20-2', 'WARN')
            return render(request, 'error.html')
        # （３）アカウント種別をチェックする。　例　１：都道府県、２：本省、３：運用業者
        if accountType == '1' or accountType == '2' or accountType == '3':
            pass
        else:
            print_log('[WARN] P25.QuestionSendListView関数 P25A20-3', 'WARN')
            return render(request, 'error.html')
        # （４）アカウントＩＤをチェックする。　例　01hokkai
        if accountId is None:
            print_log('[WARN] P25.QuestionSendListView関数 P25A20-4', 'WARN')
            return render(request, 'error.html')
        else:
            pass
        # （５）調査実施年をチェックする。　例　2016
        if operationYear is None:
            print_log('[WARN] P25.QuestionSendListView関数 P25A20-5', 'WARN')
            return render(request, 'error.html')
        else:
        ...
        if filterAccount is None:
            print_log('[WARN] P25.QuestionSendListView関数 P25A20-6', 'WARN')
            return render(request, 'error.html')
        else:
            pass
        # （７）フィルタ種別をチェックする。　例　既読 ＋ 未読
        if filterRead is None:
            print_log('[WARN] P25.QuestionSendListView関数 P25A20-7', 'WARN')
            return render(request, 'error.html')
        else:
        ...
        # ヒント：チェックに引っかかった場合、エラー画面をブラウザに戻す。
        ##########################################
        print_log('[INFO] P25.QuestionSendListView関数 P25A30', 'INFO')
        # （１）ＤＢにアクセスし、アカウントデータを取得する。
        ...
        try:
            ...
            localAccountModel = None
            print_log('[ERROR] P25.QuestionSendListView関数 P25A30', 'ERROR')
            print_log(sys.exc_info()[0], 'ERROR')
            print_log('[ERROR] P25.QuestionSendListView関数で警告が発生しました。', 'ERROR')
            print_log('[ERROR] P25.QuestionSendListView関数を警告終了しました。', 'ERROR')
            return render(request, 'error.html')
        # （２）ＤＢからの戻り値をチェックする。
        ...
        if localAccountModel != None:
            ...
            print_log('[WARN] P25.QuestionSendListView関数P25A30', 'WARN')
        ...
        if accountType == localAccountModel.ACCOUNT_TYPE:
            ...
            print_log('[WARN] P25.QuestionSendListView関数 P25A30', 'WARN')
            print_log('[WARN] accountType != localAcountModel.ACCOUNT_TYPE', 'WARN')
            return render(request, 'error.html')
        ##########################################
        # ＤＢアクセス処理、問合せ回答の送信一覧の取得処理（Ｐ２５Ａ４０）
        # （１）問合せデータ、回答データの送信一覧を格納する局所変数を初期化する。
        # （２）ＤＢにアクセスし、問合せデータ、回答データの送信一覧を取得する。
        # （３）ＤＢにアクセスし、アカウントデータを取得する。
        ##########################################
        print_log('[INFO] P25.QuestionSendListView関数 P25A40', 'INFO')
        # （１）問合せデータ、回答データの送信一覧を格納する局所変数を初期化する。
        ...
        try:
            ...
            if filterRead == '5':
                ...
                if filterAccount == '0':
                    ...
                    # ヒント：この送信トレイでは、全アカウントとは、受信者のアカウントが全アカウントのことをいう。
                    # ヒント：送信者のアカウントは、当然絞り込み条件とする。
                    ##########################################
                    ...
                    # ヒント：この送信トレイでは、特定のアカウントとは、受信者のアカウントが特定のアカウントのことをいう。
                    # ヒント：送信者のアカウントは、当然絞込条件とする。
                    ##########################################
                    localQuestionArray = QuestionModel.objects.raw("""
                        SELECT 
                            id,
            ...
            localQuestionArray = None
            print_log('[ERROR] P25.QuestionSendListView関数 P25A40', 'ERROR')
            print_log(sys.exc_info()[0], 'ERROR')
            print_log('[ERROR] P25.QuestionSendListView関数で警告が発生しました。', 'ERROR')
            print_log('[ERROR] P25.QuestionSendListView関数を警告終了しました。', 'ERROR')
        # （３）ＤＢにアクセスし、アカウントデータを取得する。
        ...
        # （１）局所変数のフォームを初期化する。
        ##########################################
        print_log('[INFO] P25.QuestionSendListView関数 P25A50', 'INFO')
        ##########################################
        # レスポンスセット処理（Ｐ２５Ａ６０）
        # （１）局所変数のレスポンス変数にブラウザに送付するデータをセットする。
        ##########################################
        print_log('[INFO] P25.QuestionSendListView関数 P25A60', 'INFO')
        # （１）局所変数のレスポンス変数にブラウザに送付するデータをセットする。
        if accountType == '1':
            ...
            if localQuestionArray != None:
                response = {
                    ...
                    'questionArray': localQuestionArray,   # 問合せ回答データの送信一覧
                    'questionLength': len(list(localQuestionArray)), # 問合せ回答データの送信一覧に含まれる問合せ回答データの件数
                ...
                response = {
                    ...
                    'message': 'message',                  # ブラウザに表示するメッセージを必要に応じてここにセットする。
                    'questionArray': localQuestionArray,   # 問合せ回答データの送信一覧
                    'questionLength': 0,                   # 問合せ回答データの送信一覧に含まれる問合せ回答データの件数
                    'accountArray': localAccountArray,     # アカウント一覧（宛先選択用）
                    'accountLength': len(list(localAccountArray)), # アカウント一覧のアカウントデータの件数
                    'filterAccount': urlquote(filterAccount), # フィルタ種別
                    'filterRead': urlquote(filterRead),    # フィルタ種別
            ...
            print_log('[ERROR] P25.QuestionSendListView関数で異常が発生しました。', 'ERROR')
            print_log('[ERROR] P25.QuestionSendListView関数を異常終了しました。', 'ERROR')
            return render(request, 'error.html')
        print_log('[INFO] P25.QuestionSendListView関数が正常終了しました。', 'INFO')
        ##########################################
        # レンダリング処理、戻り値セット処理（Ｐ１０Ａ７０）
        # （１）レンダリング関数をコールする。
        # （２）レンダリング関数の戻り値をブラウザへの戻り値にセットする。
        ##########################################
        print_log('[INFO] P25.QuestionSendListView関数 P25A70', 'INFO')
        # （１）レンダリング関数をコールする。
        # （２）レンダリング関数の戻り値をブラウザへの戻り値にセットする。
        if accountType == '1':
            # ログインした利用者のアカウント種別・区分＝１：都道府県の場合、、、
            return render(request, 'P25QuestionSendListTemplate.html', response)
        elif accountType == '2':
            # ログインした利用者のアカウント種別・区分＝２：本省の場合、、、
            return render(request, 'P25QuestionSendListTemplate.html', response)
        elif accountType == '3':
        ...
        ##########################################
        # （１）ログ出力関数をコールし、ログファイルにエラーメッセージを出力する。
        print_log('[ERROR] P25.QuestionSendListView関数 P25A80', 'ERROR')
        print_log(sys.exc_info()[0], 'ERROR')
        print_log('[ERROR] P25.QuestionSendListView関数でエラーが発生しました。', 'ERROR')
        print_log('[ERROR] P25.QuestionSendListView関数が異常終了しました。', 'ERROR')
        ##########################################
        # 戻り値セット処理（Ｐ２５Ａ９０）
        # （１）レンダリング関数をコールする。
        # （２）レンダリング関数の戻り値をブラウザへの戻り値にセットする。
        ##########################################
        print_log('[ERROR] P25.QuestionSendListView関数 P25A90', 'ERROR')
        # （１）レンダリング関数をコールする。
        # （２）レンダリング関数の戻り値をブラウザへの戻り値にセットする。
        return render(request, 'error.html')